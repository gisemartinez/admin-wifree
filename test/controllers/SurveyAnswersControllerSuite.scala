...
import play.test.Helpers.{POST, GET, contentAsString}
    ...
    val json: JsValue = Json.parse(s"""
{
  ...
  """)
    ...
    val answerAsCSVRequest =
      Helpers.fakeRequest
        .method(GET)
        .session(session)
        .uri(s"/api/survey/${survey.getId}/answers")

    val answerAsCSVResult = Helpers.route(WiFreeSuite.app, answerAsCSVRequest)

    assertEquals(OK, answerSurveyResult.status)
    assertEquals(OK, answerAsCSVResult.status)

    val answerSurveyResultStr = contentAsString(answerSurveyResult)
    val answerAsCSVResultStr = contentAsString(
      answerAsCSVResult,
    ...
    assertTrue(
      answerAsCSVResultStr.contains(
        """id,title,question_number,question,answer
         |1,This is a survey with 1 field to answer,1,label 1,14
         |1,This is a survey with 1 field to answer,2,label 2,OptionA
         |""".stripMargin
      )
    )
  }
